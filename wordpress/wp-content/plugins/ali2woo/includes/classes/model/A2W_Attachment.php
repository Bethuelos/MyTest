...
if (!class_exists('A2W_Attachment')) {
    ...
    class A2W_Attachment {
        ...
        public function create_attachment($post_id, $image_path, $params = array()) {
            ...
            if ($check_duplicate) {
                $old_attachment_id = $wpdb->get_var($wpdb->prepare("SELECT ID FROM $wpdb->posts p INNER JOIN $wpdb->postmeta pm on (p.ID = pm.post_id) WHERE p.post_parent=%d and pm.meta_key='_a2w_external_image_url' AND meta_value=%s LIMIT 1", $post_id, $image_path));
        ...
        public static function calc_total_external_images() {
            ...
            $cnt += $wpdb->get_var("select count(ID) from $wpdb->posts where post_type = 'attachment' AND post_content LIKE '%.alicdn.com%'");
        ...
        public static function find_external_images($page_size = 1000) {
            global $wpdb;
            $result_ids = array();
            $tmp_product_ids = $wpdb->get_results("select distinct p1.id as id from $wpdb->posts p1 LEFT join $wpdb->postmeta pm1 on(p1.id = pm1.post_id) where p1.post_type = 'attachment' and pm1.meta_key='_wp_a2w_attached_file' and pm1.meta_value='1' LIMIT $page_size", ARRAY_N);
            foreach ($tmp_product_ids as $row) {
                $result_ids[] = $row[0];
            }

            $tmp_product_ids = $wpdb->get_results("select ID from $wpdb->posts where post_type = 'attachment' AND post_content LIKE '%alicdn.com%'", ARRAY_N);
            foreach ($tmp_product_ids as $row) {
                $result_ids[] = $row[0];
            }
            ...
            if ($post_id) {
                ...
                if ($post->post_type === 'attachment') {
                    ...
                    if ($tmp && intval($tmp) === 1) {
                        ...
                        $new_image_id = $this->create_attachment($post->post_parent, $post->guid, array('inner_post_id' => $post_id, 'title' => $post->post_title, 'alt' => $post->post_title, 'check_duplicate' => false));
                        if ($new_image_id) {
                    ...
                    for ($i = $elements->length; --$i >= 0;) {
                        ...
                        if (strpos($old_url, '.alicdn.com') !== false) {
                            $attachment_id = $this->create_attachment($post_id, $e->getAttribute('src'), $post->guid, array('inner_post_id' => $post_id, 'title' => $post->post_title, 'alt' => $post->post_title, 'check_duplicate' => false));
