...
function fill_modal_shipping_info(product_id, country, items, page = 'search', default_method = '', onSelectCallback = null) {
    const tmp_data = { product_id, country, 'shipping': items, page, default_method, onSelectCallback };
    jQuery(".modal-shipping").data(tmp_data);
    jQuery('#modal-country-select').val(country).trigger('change');
    ...
    // const product_block = jQuery('.product-card[data-id="' + tmp_data.product_id + '"]');
    // const product_block_import = jQuery('.product[data-id="' + tmp_data.product_id + '"]');

    // if (min_shipping_price) {
    //     jQuery(product_block).find('.product-card-shipping-info .shipping-title').html(min_shipping_price.formated_price + ' ' + min_shipping_price.name);
    //     jQuery(product_block).find('.product-card-shipping-info .delivery-time').html(min_shipping_price.time + ' days');
    //     jQuery(product_block).find('.product-card-shipping-info').data({ 'country': tmp_data.country, 'shipping': tmp_data.shipping });

    //     if (product_block_import.length > 0) {
    //         jQuery(product_block_import).find('.external-shipping').html(jQuery(product_block_import).find('.external-shipping').attr('data-currency') + min_shipping_price.price);
    //         jQuery(product_block_import).find('.external-shipping').attr('data-value', min_shipping_price.price);
    //         jQuery(product_block_import).find('.shipping-country').html(jQuery("#modal-country-select option:selected").text());

    //         const product_data = jQuery(product_block_import).data();
    //         product_data.country = country;
    //         product_data.shipping = items;
    //         jQuery(product_block_import).data(product_data);

    //         a2w_calc_profit(jQuery(product_block_import).attr('data-id'));
    //     }
    // }
}

function a2w_load_shipping_info(product_id, country, page = 'search', callback = null) {
    var data = { 'action': 'a2w_load_shipping_info', 'id': product_id, 'country': country, 'page': page };
    ...
    jQuery.post(ajaxurl, data).done(function (response) {
        ...
        if (callback) {
            callback(json.state, json.products.length > 0 ? json.products[0].items : [], json.products[0].default_method, json.products[0].shipping_cost, json.products[0].variations)
        }
    }).fail(function (xhr, status, error) {
        console.log(error);
    });
...
function a2w_update_product_shipping_info(product_id) {
    ...
    const loadCallback = function (state, items, default_method, shipping_cost, variations) {
        if (state != 'error') {
            ...
            // jQuery.each(variations, function (_, v) {
            //     jQuery('.variants-table [data-id="' + v.id + '"] .price').val(v.calc_price)
            //     jQuery('.variants-table [data-id="' + v.id + '"] .regular_price').val(v.calc_regular_price)
            // });


            // var min_shipping_price = find_min_shipping_price(items, default_method);
            // if (min_shipping_price) {
            //     jQuery(product).find('.external-shipping').html(jQuery(product).find('.external-shipping').attr('data-currency') + min_shipping_price.price);
            //     jQuery(product).find('.external-shipping').attr('data-value', min_shipping_price.price);
            //     a2w_calc_profit(product_id);
            // }
        }
    }

    a2w_load_shipping_info(product_id, product_data.country, 'import', loadCallback)
}
...
(function ($, window, document, undefined) {
    $(function () {
        ...
        $(".product .shipping-country").on("click", function () {
            ...
            const onSelectCallback = function (product_id, items, country, method) {
                const data = { 'action': 'a2w_set_shipping_info', 'id': product_id, country, method };
                jQuery.post(ajaxurl, data).done(function (response) {
                    ...
                    if (json.state !== 'ok') {
                        ...
                        jQuery(product).data({ ...product_data, shipping: items, default_method: json.default_method });
                    }
                }).fail(function (xhr, status, error) {
                    console.log(error);
                });
            ...
            if (!product_data.country || !product_data.shipping) {
                fill_modal_shipping_info($(this).parents(".product").attr('data-id'), "", null, 'import', '', onSelectCallback);
            } else {
                fill_modal_shipping_info($(this).parents(".product").attr('data-id'), product_data.country, product_data.shipping, 'import', product_data.default_method, onSelectCallback);
            }
        ...
        $('#modal-country-select').on('change', function (evt) {
            ...
            if ($(this).val() && $(this).val() != "" && shipping_data.country != $(this).val()) {
                shipping_data.country = $(this).val();
                ...
                a2w_load_shipping_info(shipping_data.product_id, shipping_data.country, shipping_data.page, function (state, items, default_method, shipping_cost, variations) {
                    if (state != 'error') {
                        fill_modal_shipping_info(shipping_data.product_id, shipping_data.country, items, shipping_data.page, default_method, shipping_data.onSelectCallback);
                        if (shipping_data.onSelectCallback) {
                            shipping_data.onSelectCallback(shipping_data.product_id, items, shipping_data.country, default_method)
                        }
                    }
                })
            }
        ...
        $('.modal-shipping').on('change', '.select_method', function () {
            ...
            waitForFinalEvent(function () {
                if (shipping_data.onSelectCallback) {
                    shipping_data.onSelectCallback(shipping_data.product_id, shipping_data.shipping, shipping_data.country, _this_value)
                }
